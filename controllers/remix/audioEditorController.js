...
const audioController = {
  ...
  uploadAudioFiles: async (req, res) => {
    if (req.user.role !== 'admin' && req.user.role !== 'participant') {
      return res.status(403).json({ message: 'You do not have permission to upload audio files.' });
    }
    try {
  ...
  getAudioFiles: async (req, res) => {
    // Admin and coordinator can see all files
    if (req.user.role === 'admin' || req.user.role === 'coordinator') {
    ...
    } else if (req.user.role === 'volunteer' || req.user.role === 'audience') {
      // Only listen, not manage
      ...
      return res.status(403).json({ message: 'You do not have permission to view audio files.' });
    }
  },
